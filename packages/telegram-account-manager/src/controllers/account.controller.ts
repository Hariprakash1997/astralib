...
export function createAccountController(
  ...
  return {
    ...
    async getById(req: Request, res: Response) {
      try {
        const account = await TelegramAccount.findById(req.params.id).select('-session');
        if (!account) {
    ...
    async getCapacity(req: Request, res: Response) {
      try {
        const capacity = await capacityManager.getAccountCapacity(req.params.id);
    ...
    async getHealth(req: Request, res: Response) {
      try {
        const health = await healthTracker.getHealth(req.params.id);
        if (!health) {
          return res.status(404).json({ success: false, error: 'Account not found' });
        }
    ...
    async remove(req: Request, res: Response) {
      try {
        const client = connectionService.getClient(req.params.id);
        if (client) {
          return res.status(400).json({ success: false, error: 'Account must be disconnected before deletion' });
        }

        const result = await TelegramAccount.findByIdAndDelete(req.params.id);
        if (!result) {
          return res.status(404).json({ success: false, error: 'Account not found' });
        }
    ...
    async connect(req: Request, res: Response) {
      try {
        await connectionService.connect(req.params.id);
        res.json({ success: true });
      } catch (error: unknown) {
        const message = error instanceof Error ? error.message : 'Unknown error';
        res.status(500).json({ success: false, error: message });
    ...
    async disconnect(req: Request, res: Response) {
      try {
        await connectionService.disconnect(req.params.id);
        res.json({ success: true });
      } catch (error: unknown) {
        const message = error instanceof Error ? error.message : 'Unknown error';
        res.status(500).json({ success: false, error: message });
    ...
    async reconnect(req: Request, res: Response) {
      try {
        await connectionService.reconnect(req.params.id);
        res.json({ success: true });
      } catch (error: unknown) {
        const message = error instanceof Error ? error.message : 'Unknown error';
        res.status(500).json({ success: false, error: message });
    ...
    async quarantine(req: Request, res: Response) {
      try {
        ...
        await quarantineService.quarantine(req.params.id, reason, durationMs);
        res.json({ success: true });
      } catch (error: unknown) {
        const message = error instanceof Error ? error.message : 'Unknown error';
        res.status(500).json({ success: false, error: message });
    ...
    async release(req: Request, res: Response) {
      try {
        await quarantineService.release(req.params.id);
        res.json({ success: true });
      } catch (error: unknown) {
        const message = error instanceof Error ? error.message : 'Unknown error';
        res.status(500).json({ success: false, error: message });
