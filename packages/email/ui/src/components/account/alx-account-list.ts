...
export class AlxAccountList extends LitElement {
  ...
  constructor() {
    super();
    this._showHelp = localStorage.getItem('alx-help-accounts') === 'true';
  }

  private _toggleHelp(): void {
    this._showHelp = !this._showHelp;
    localStorage.setItem('alx-help-accounts', String(this._showHelp));
  }
      ...
      <div class="alx-card">
        ...
        ${this._showHelp ? html`
          <div class="help-panel">
            <strong>Email Accounts</strong> are sender identities used to send emails.
            <ul>
              <li><strong>Status</strong> — Active (sending), Warmup (building reputation), Disabled (paused), Error (failed)</li>
              <li><strong>Health</strong> — Score 0-100. Drops on bounces/complaints. Below 50 = auto-disabled.</li>
              <li><strong>Total Sent</strong> — Lifetime email count from this account</li>
              <li><strong>Warmup</strong> — Gradually increases daily send limit for new accounts. "Day N" shows progress.</li>
              <li><strong>Edit</strong> — Click the pencil icon to modify account settings</li>
              <li><strong>Delete</strong> — Click X to remove (requires confirmation)</li>
            </ul>
          </div>
        ` : ''}
                          ...
                          <td>
                            <span class="progress-bar">
                              ...
                              <span class="progress-label">
                                ${a.health?.score ?? 0}
                                <span class="${(a.health?.score ?? 0) > 80 ? 'health-good' : (a.health?.score ?? 0) > 50 ? 'health-fair' : 'health-poor'}">${(a.health?.score ?? 0) > 80 ? 'Good' : (a.health?.score ?? 0) > 50 ? 'Fair' : 'Poor'}</span>
                              </span>
                            </span>
                          </td>
                          <td>
