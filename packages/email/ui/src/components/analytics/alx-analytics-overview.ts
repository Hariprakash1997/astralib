...
export class AlxAnalyticsOverview extends LitElement {
  ...
  override render() {
    ...
    return html`
      ...
      ${this._metrics.length > 0 ? html`
        <p class="summary-line">
          ${(this._metrics.find(m => m.key === 'sent')?.count ?? 0).toLocaleString()} sent,
          ${(this._metrics.find(m => m.key === 'delivered')?.count ?? 0).toLocaleString()} delivered,
          ${(this._metrics.find(m => m.key === 'opened')?.count ?? 0).toLocaleString()} opened,
          ${(this._metrics.find(m => m.key === 'clicked')?.count ?? 0).toLocaleString()} clicked
        </p>
      ` : nothing}
