...
export class AlxTelegramDashboard extends LitElement {
  ...
  private _applyTheme(): void {
    if (this.theme === 'dark') {
      ...
      for (const prop of Object.keys(AlxTelegramDashboard.DARK_VARS)) {
  ...
  override render() {
    return html`
      ...
      ${this._loadedTabs.has('inbox') ? html`
        <div class="panel ${this._activeTab === 'inbox' ? 'active' : ''}">
          <alx-tg-inbox .density=${this.density}></alx-tg-inbox>
        </div>
      ` : nothing}

      ${this._loadedTabs.has('bot-stats') ? html`
        <div class="panel ${this._activeTab === 'bot-stats' ? 'active' : ''}">
          <alx-tg-bot-stats .density=${this.density}></alx-tg-bot-stats>
        </div>
      ` : nothing}
...
safeRegister('alx-telegram-dashboard', AlxTelegramDashboard);

declare global {
  interface HTMLElementTagNameMap {
    'alx-telegram-dashboard': AlxTelegramDashboard;
  }
}
