...
// RedisLock is mocked at instance level via service['lock'] after construction
...
function createService(models = createMockModels(), config = createMockConfig(), lockAcquire = true) {
  ...
  // Mock the lock at instance level
  (service as any).lock = {
    acquire: vi.fn().mockResolvedValue(lockAcquire),
...
describe('RuleRunnerService', () => {
  beforeEach(async () => {
    vi.clearAllMocks();
    // RedisLock is now mocked at instance level in createService()
  ...
  describe('runAllRules', () => {
    it('skips when lock cannot be acquired', async () => {
      const { service, models, config } = createService(undefined, undefined, false);
