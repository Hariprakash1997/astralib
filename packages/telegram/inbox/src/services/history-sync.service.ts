...
import { DEFAULT_HISTORY_SYNC_LIMIT } from '../constants';
  ...
  async syncChat(accountId: string, chatId: string, limit?: number): Promise<SyncResult> {
    ...
    try {
      ...
      for (const msg of messages) {
        ...
        // Skip if message already exists (upsert pattern)
        const existing = await this.TelegramMessage.findOne({ messageId });
        if (existing) {
          oldestId = msg.id;
          continue;
        }
        ...
        try {
          await this.TelegramMessage.create({
            ...
            senderType: isOutbound ? 'account' : 'user',
  ...
  private extractContentType(msg: Api.Message): ContentType {
    ...
    if (msg.media instanceof Api.MessageMediaDocument) {
      ...
      if (doc instanceof Api.Document) {
        ...
        if (mimeType.includes('webp') || mimeType.includes('tgs')) return 'sticker';
      }
      return 'document';
    }
