...
import { formatDate, formatDuration, defaultDateFrom, defaultDateTo } from '../utils/format.js';
...
function renderStatusBadge(status?: string) {
  ...
  const map: Record<string, string> = {
    completed: 'alx-badge-success',
    running: 'alx-badge-info',
    failed: 'alx-badge-danger',
    cancelled: 'alx-badge-warning',
    pending: 'alx-badge-default',
  };
...
export class AlxRunHistory extends LitElement {
  ...
  @state() private _activeRunProgress: {
    matched?: number;
    sent?: number;
    skipped?: number;
    throttled?: number;
  ...
  private async _pollRunStatus(): Promise<void> {
    ...
    try {
      ...
      this._activeRunProgress = {
        ...
        throttled: progress.throttled ?? 0,
  ...
  private async _loadLogs(): Promise<void> {
    ...
    try {
      ...
      this._logs = (res.logs ?? []).map((log: any) => ({
        ...
        totalMatched: log.totalMatched ?? log.totalStats?.matched ?? 0,
        totalSent: log.totalSent ?? log.totalStats?.sent ?? 0,
        totalSkipped: log.totalSkipped ?? log.totalStats?.skipped ?? 0,
        totalThrottled: log.totalThrottled ?? log.totalStats?.throttled ?? 0,
        totalFailed: log.totalFailed ?? log.totalStats?.failed ?? 0,
  ...
  private async _onRunNow(): Promise<void> {
    ...
    try {
      ...
      if (runId && runId !== 'undefined') {
        ...
        this._reloadTimer = setTimeout(() => this._loadLogs(), 2000);
      }
    } catch (err) {
    ...
    return html`
      <div class="alx-card">
        ...
        <div class="toolbar">
          ...
          <button class="alx-btn-sm" ?disabled=${this._triggering || !!this._activeRunId} @click=${this._onRunNow}>
            ${this._triggering ? 'Running...' : 'Run Now'}
          </button>
        </div>

        ${this._error ? html`<div class="alx-error">${this._error}</div>` : nothing}
        ${this._successMsg ? html`<div class="alx-success-msg">${this._successMsg}</div>` : nothing}
          ...
          <div class="run-progress">
            <div class="run-progress-header">
              ...
              ${this._activeRunProgress.status !== 'starting' ? html`
                <span class="run-progress-stats">
                  ...
                  ${this._activeRunProgress.throttled ?? 0} throttled,
                  ${this._activeRunProgress.failed ?? 0} failed
            ...
            ${this._activeRunProgress.matched && this._activeRunProgress.matched > 0 ? html`
              <div class="run-progress-bar">
                <div class="run-progress-fill" style="width:${Math.min(100, Math.round(
                  ((this._activeRunProgress.sent ?? 0) +
                   (this._activeRunProgress.skipped ?? 0) +
                   (this._activeRunProgress.throttled ?? 0) +
                   (this._activeRunProgress.failed ?? 0)) /
                  this._activeRunProgress.matched * 100
                ))}%"></div>
              </div>
            ` : nothing}
          </div>
  ...
  <p>Runs appear here after rules are executed.</p>
  <p>Set up templates and rules first — then click Run Now.</p>
</div>`
            : html`
                <table>
                  <thead>
                    ...
                    ${this._logs.map((log) => {
                      ...
                      return html`
                        <tr class="expandable" @click=${() => this._toggleExpand(log._id)}>
                          ...
                          <td class="stat">${log.totalMatched}</td>
                          <td class="stat">${log.totalSent}</td>
                          <td class="stat">${log.totalSkipped}</td>
                          <td class="stat">${log.totalThrottled}</td>
                          <td class="stat">
                            ${log.totalFailed > 0
                              ? html`<span class="alx-badge alx-badge-danger">${log.totalFailed}</span>`
                              : '0'}
                          </td>
                          <td>
                            ${log.status === 'running'
                                    ...
                                    <tbody>
                                      ${log.perRuleStats.map(
                                        (stat) => html`
                                          <tr>
                                            <td>${stat.ruleName}</td>
                                            <td class="stat">${stat.matched}</td>
                                            <td class="stat">${stat.sent}</td>
                                            <td class="stat">${stat.skipped}</td>
                                            <td class="stat">${stat.throttled}</td>
                                            <td class="stat">${stat.failed}</td>
                                          </tr>
                                        `,
                                      )}
                                    </tbody>
