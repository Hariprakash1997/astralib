...
export function createTemplateController(templateService: TemplateService, options?: TemplateControllerOptions) {
  ...
  const create = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!name || !bodies || bodies.length === 0 || !category || !audience || !platform) {
      return res.status(400).json({ success: false, error: 'name, bodies, category, audience, and platform are required' });
    }
  ...
  const sendTest = asyncHandler(async (req: Request, res: Response) => {
    const { testEmail, sampleData } = req.body;
    if (!testEmail) {
      return res.status(400).json({ success: false, error: 'testEmail is required' });
    }
    const result = await templateService.sendTest(getParam(req, 'id'), testEmail, sampleData || {});
    if (!result.success) {
  ...
  return { list, getById, create, update, remove, toggleActive, preview, previewRaw, validate, sendTest, clone, previewWithRecipient };
}
