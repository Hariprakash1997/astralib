...
export type RunLogDocument = HydratedDocument<IRunLog>;

export interface RunLogStatics {
  getRecent(limit?: number): Promise<RunLogDocument[]>;
  getByRuleId(ruleId: string | Types.ObjectId, limit?: number): Promise<RunLogDocument[]>;
}

export type RunLogModel = Model<IRunLog> & RunLogStatics;

export function createRunLogSchema(collectionPrefix?: string) {
  ...
  const schema = new Schema<IRunLog>(
    {
    ...
    {
      collection: `${collectionPrefix || ''}run_logs`,
